	    ...
	    <div class="row ${pageBean.totalCount<=0?"div1":"" }">
	    	...
	    	<c:forEach items="${pageBean.data}" var="g" varStatus="i">
		    	<div class="col-sm-3">
				    <div class="thumbnail">
				      ...
				      <div class="caption">
				        <h4>商品名称<a href="${pageContext.request.contextPath}/goods/getgoodsdetailbyid?id=${g.id}">${g.name}</a></h4>
				        <p>热销指数
			...
			<nav aria-label="..." class="text-center">
			  <ul class="pagination">
			  	...
			  	<c:if test="${pageBean.pageNum>1 }">
			  	 	<li><a href="${pageContext.request.contextPath }/goods/getgoodsbytypeid?pageNum=${pageBean.pageNum-1}&pageSize=${pageBean.pageSize}&typeId=${typeId}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
			  	</c:if>
			    ...
			    <c:forEach var="pn" begin="${pageBean.startPage }" end="${pageBean.endPage }" step="1">
			    	...
			    	<c:if test="${pn!=pageBean.pageNum }">
			    		<li ><a href="${pageContext.request.contextPath }/goods/getgoodsbytypeid?pageNum=${pn }&pageSize=${pageBean.pageSize}&typeId=${typeId}">${pn }</a></li>
			    	</c:if>
			    </c:forEach>
			    ...
			    <c:if test="${pageBean.pageNum==pageBean.totalPage }">
			     <li class="disabled"><span aria-hidden="true">»</span></li>
			      ...
			      <c:if test="${pageBean.pageNum<pageBean.totalPage }">
			     <li><a href="${pageContext.request.contextPath }/goods/getgoodsbytypeid?pageNum=${pageBean.pageNum+1}&pageSize=${pageBean.pageSize}&typeId=${typeId}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
			    </c:if>
