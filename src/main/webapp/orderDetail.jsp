...
<script type="text/javascript">
	function pay(orderId,money){
		location.href="${pageContext.request.contextPath}/pay.jsp?oid="+orderId+"&omoney="+money;
	}
	function payWeiXin(orderId,money){
		location.href="${pageContext.request.contextPath}/payWeixin.jsp?oid="+orderId+"&omoney="+money;
	}
</script>
</head>
	...
	<table cellpadding="0" cellspacing="0" align="center" width="100%" class="table table-striped table-bordered table-hover">
		...
		<tr>
			<td align="right" colspan="4" style="margin-right: 40px;">
				<a href="${pageContext.request.contextPath }/order/getorderlist" class="btn btn-danger btn-sm">返回订单列表</a>
				&nbsp;&nbsp;
				<c:if test="${order.status eq 1 }">
					<button type="button" onclick="pay('${order.id}','${order.money}')" class="btn btn-warning btn-sm">易宝支付</button>
						&nbsp;&nbsp;
