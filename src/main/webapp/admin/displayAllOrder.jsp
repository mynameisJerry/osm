	...
	<script type="text/javascript">
        ...
        $(function(){
            $("#search").click(function(){
                ...
                location.href="${pageContext.request.contextPath}/order/searchOrder?username="+username+"&status="+status+"&num="+Math.random();
            })
        })
	</script>
</head>
			...
			<div class="panel-body">
				...
				<nav aria-label="..." class="text-center">
					<ul class="pagination">
						...
						<c:if test="${pageBean.pageNum>1 }">
							<li><a href="${pageContext.request.contextPath }/order/getallorder?pageNum=${pageBean.pageNum-1}&pageSize=${pageBean.pageSize}&username=${username}&status=${status}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
						</c:if>
						...
						<c:forEach var="pn" begin="${pageBean.startPage }" end="${pageBean.endPage }" step="1">
							...
							<c:if test="${pn!=pageBean.pageNum }">
								<li ><a href="${pageContext.request.contextPath }/order/getallorder?pageNum=${pn }&pageSize=${pageBean.pageSize}&username=${username}&status=${status}">${pn }</a></li>
							</c:if>
						</c:forEach>
						...
						<c:if test="${pageBean.pageNum<pageBean.totalPage }">
							<li><a href="${pageContext.request.contextPath }/order/getallorder?pageNum=${pageBean.pageNum+1}&pageSize=${pageBean.pageSize}&username=${username}&status=${status}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
						</c:if>
