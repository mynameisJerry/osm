...
public class LogAspect {
    ...
    @Pointcut("execution(* com.pzhu.shopping.myshop.controller..*(..))")
        ...
        for (Method method : methods) {
            if (method.getName().equalsIgnoreCase(name1)) {
                ...
                if (logAnnotation != null) {
                    ...
                    String operationtype = logAnnotation.operationType();
                    String loginname=StringUtil.getRandomString(10);
                    System.out.println(opertionname + "被执行了(" + operationtype + "),参数是:" + Arrays.toString(args));
        ...
        for (Method method : methods) {
            if (method.getName().equalsIgnoreCase(name1)) {
                ...
                if (logAnnotation != null) {
                    ...
                    String operationtype = logAnnotation.operationType();
                    String loginname=StringUtil.getRandomString(10);
                    System.out.println(opertionname + "被执行了(" + operationtype + "),参数是:" + Arrays.toString(args) +"结果失败了!");
